...
foreach ($file in $files) {
    ...
    if (($file.Name -clike "Screenshot*") -or ($file.Name -clike "IMG_*") -or ($file.Name -clike "IMG-*") -or ($file.Name -clike "VID_*") -or ($file.Name -clike "VID-*") -or ($file.Name -match "\d{8}_\d{6}")) {
    ...
    if ("" -ne $dateTakenTemp) {
        ...
        # Skip files with a "dateTaken" year of 1999 or older
        if ($dateTaken.Year -le 1999) {
            Write-Host "Skipping $($file.FullName) because the dateTaken year is 1999 or older" -ForegroundColor Yellow
            continue
        }
